...
from ..UndoRedoOperations.renew_warranty_operation import WarrantyOperation
...
class CarService:
    ...
    @update_undo_redo
    def update(new_car: Car, id: int, undoredo: bool = True):
        try:
            car = Car.objects.get(id=id)
        except Car.DoesNotExist as e:
            raise Car.DoesNotExist(f'Error: {str(e)}')
    ...
    def renewWarranty(undoredo: bool = True):
        ...
        cars_with_renewed_warranty = []

        for car in cars:
            difference = relativedelta(today, car.acquisition_date)
            if (difference.years < 3 and
                    car.kilometers < 60000 and car.warranty is False):
                car.warranty = True
                car.save()
                cars_with_renewed_warranty.append(car)

        if undoredo:
            UndoRedoService.addUndoOperation(
                WarrantyOperation(cars_with_renewed_warranty))
